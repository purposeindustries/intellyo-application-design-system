...
describe('FEF tags input tests', () => {
  ...
  it('Checks the the fef buttons page title and browser compare visual regression', () => {
    ...
    assert(takeScreenshotAndGetWholePageCompareResult(5), 'Whole FEF tags input page screenshot compare to a reference picture');
  });
  ...
  it('should check the input: ' + TEST_NAME_TAGS_INPUT_NAME, () => {
    ...
    assert(takeScreenShotOfElement(TAGS_INPUT_WHOLE_ORIG_XPATH, 10), 'Tags input is not similar to the reference');
  });
  ...
  it('should check the input: ' + TEST_NAME_TAGS_INPUT_MOUSE_OVER, () => {
    if (!browser.desiredCapabilities.browserName.includes('firefox')) { //TODO: We should find solutions for moveToObject and other cross-browser problems
      ...
      assert(takeScreenShotOfElement(TAGS_INPUT_WHOLE_ORIG_XPATH, 10), 'Mouse over tags input is not similar to the reference');
    }
  });
  ...
  it('should check the input: ' + TEST_NAME_TAGS_INPUT_CLICK, () => {
    ...
    assert(takeScreenShotOfElement(TAGS_INPUT_WHOLE_CLICKED_XPATH, 10), 'Clicked tags input is not similar to the reference');
  });
  ...
  it('should check the input: ' + TEST_NAME_TAGS_ADD_INPUT, () => {
    ...
    assert(takeScreenShotOfElement(TAGS_INPUT_WHOLE_CLICKED_XPATH, 11), 'Added tags input is not similar to the reference');
  });
  ...
  it('should check the input: ' + TEST_NAME_TAGS_ADD_INPUT_CLICK, () => {
    ...
    assert(takeScreenShotOfElement(TAGS_INPUT_WHOLE_ORIG_XPATH, 11), 'Added and clicked tags input is not similar to the reference');
  });
});
