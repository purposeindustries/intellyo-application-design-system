...
describe('FEF buttons tests', () => {
  ...
  it.only('should check the button: ' + SAVE_BUTTON_NAME, () => {
    ...
    assert(takeScreenShotOfElement(SAVE_BUTTON_XPATH, 5, true), 'Save button is not similar to the reference before click');
    browser.click(SAVE_BUTTON_XPATH);

    $(SAVE_BUTTON_SELECTOR).waitForExist(8500);
    assert(takeScreenShotOfElement(SAVE_BUTTON_XPATH, 5, true) + '2nd check if save button is similar to the reference');
  });
