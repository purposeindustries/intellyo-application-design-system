...
//Selectors/xpaths
const SELECT_HELLO_XPATH = '//*[@class="card"][1]//*[@class="card-body"]//*[@class="dropdown select"][2]';
...
describe('FEF select tests', () => {
  ...
  it('should check the input ' + SELECT_HELLO, () => {
    browser.url('/select');
    assert(browser.isExisting(SELECT_HELLO_XPATH), 'Select "Hello" is not existing in the DOM');
    assert(takeScreenShotOfElement(SELECT_HELLO_XPATH,
      {firefoxTolerance: 15, defaultTolerance: 5.5, ignoreComparison: false}),
  ...
  it('should check the input ' + SELECT_HELLO_OPENED, () => {
    browser.url('/select');
    assert(browser.isExisting(SELECT_HELLO_XPATH), 'Select "Hello" is not existing in the DOM');
    browser.click(SELECT_HELLO_XPATH);
